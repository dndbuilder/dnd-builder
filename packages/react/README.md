...
Before using the Editor component, you need to create an app in the dashboard to get your `appId` and `appKey` credentials.
...
1. Sign up or log in to the [DnD Builder Dashboard](https://dndbuilder.com/dashboard)
2. Create a new app from the App Management section
3. Copy your `appId` and `appKey` (the appKey is shown only once, so save it securely)
4. Use these credentials to initialize the Editor component

**Note**: The Editor component validates your credentials on initialization. If validation fails, an error overlay will be displayed.
...
**Required Props:**
- `appId`: Your application ID (obtained from the dashboard)
- `appKey`: Your application key (obtained from the dashboard)
- `content`: The initial content structure

The Editor validates your `appId` and `appKey` credentials on mount. If validation fails, an error overlay will be displayed.
